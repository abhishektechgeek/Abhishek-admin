...
class ServiceTimeSlot extends CI_Controller {   
...
* @see http://codeigniter.com/user_guide/general/urls.html
	...
	function servicetimeslotlist(){
		...
		$this->load->view('servicetimeslot',$this->view_data);
	  ...
	  if($this->view_data['is_modify'] == false){
		...
		$data['segment']=$this->servicemodel->getSegmentList();
  		...
  		if($edit_id!=""){			
			...
			$this->nocache();
		$data['page_title']="Edit Service Time Slot ";
		$this->load->view('edit_service_time_slot',$data);	
 	     $this->load->view('footer');
	}
	...
	if (!empty($_FILES['image']['name'])){
 	...
 	/*	
      ...
      function update_time_slot(){
  		    ...
  		    $timeInterval=$this->uri->segment(5);
                   
 			$success=$this->servicemodel->updatetimeslot($id,$time,$timeInterval);
			if($success){
 ...
 function update_service_order(){
	$orderArray=$_GET['ajaxid'];
	//echo  $_GET['segid'];
	
	$pieces = explode("&", $orderArray);
//print_r($pieces);
	//print_r($orderArray);
	$count = 1;
	 foreach ($pieces as $idval) 
	{
                      //  echo $idval."</br>";
						$ids = explode("=", $idval);
						//print_r($ids);
						//echo $ids[1]
		//echo $query = "UPDATE skills SET order_by = " . $count . " WHERE id = " . $ids[1] ." and parent_id =".$_GET['segid']."</br>";
		//mysql_query($query) or die('Error, insert query failed');
			
	   $this->servicemodel->updateServiceOrder($count,$_GET['segid'],$ids[1]);
	   $count ++;
	}
        ...
        function update_service_type(){
 			...
 			$success=$this->servicemodel->updateServiceType($id,$value);
			if($success){
