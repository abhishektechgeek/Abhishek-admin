...
* @see http://codeigniter.com/user_guide/general/urls.html
	...
	function categorylist(){ //echo "lkjkljlkjl";
 ...
 public	function nocache()
	...
	if($this->view_data['is_modify'] == false){
		...
		//$data['user_detail']=$this->adminmodel->fatch_admin_profile($id);
	...
	function save_category(){
	 ...
	 if($this->view_data['is_modify'] == false){
		...
		if($success){
			  ...
			  // redirect(base_url().'category/categorylist');	
			     echo '<script type="text/javascript">self.close(); opener.location.href = "http://gofavo.com/api/admin/category/categorylist/";</script>';	 
			}else{
	...
	if($this->view_data['is_modify'] == false){
		   $this->segmentlist();
  		    ...
  		    $edit_id=$this->uri->segment(3); 
                   // $this->load->model('segmentmodel');
 			$success=$this->categorymodel->inactiveSegment($edit_id);
			if($success){
                            $this->nocache();
 			   $this->session->set_flashdata('success','This record has been inactivated');	
	...
	if($this->view_data['is_modify'] == false){
		   $this->segmentlist();
  		   ...
  		   $edit_id=$this->uri->segment(3); 
 			$success=$this->categorymodel->activeSegment($edit_id);
			if($success){
	...
	if($this->view_data['is_modify'] == false){
		...
		if ($this->upload->do_upload('image')){
               ...
               $this->session->set_flashdata('fail','This image can not uploaded');		
			   redirect(base_url().'category/edit_segment/'.$this->input->post('id').'/'.$this->input->post('parent_id'));
             }
  		}
		else{
