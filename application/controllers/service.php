...
class Service extends CI_Controller {
 ...
 $this->view_data['dropdownvalue'] =  $this->servicemodel->dropdownlist();
        ...
        $serviceType = array('0'=>'SELECT','1' => 'HOURLY', '2' => 'Fixed Price');
    ...
    function edit_service() {
      ...
      function edit_service_image() {
        ...
        $this->nocache();
        $data['page_title'] = "Edit service";
        $this->load->view('edit_service_image', $data);
        $this->load->view('footer');
    }
   ...
   if ($this->view_data['is_modify'] == false) {
        ...
        if (isset($_POST['submit'])) {
            ...
            if (!empty($_FILES['image']['name'])) {
                ...
                if ($this->upload->do_upload('image')) {
                  ...
                  $segmentid= $this->input->post('segmentId');
                     echo '<script type="text/javascript">self.close(); opener.location.href = "http://gofavo.com/api/admin/category/edit_category/'.$serviceid/$segmentid.'</script>';	 
                }
            } else {
            ...
            if ($success) {
                ...
                //redirect(base_url() . 'service/servicelist');
                 $segmentid= $this->input->post('segmentId');
                   echo '<script type="text/javascript">self.close(); opener.location.href = "http://gofavo.com/api/admin/service/servicelist/'.$segmentid.'" </script>';	 
            } else {
   ...
   if ($this->view_data['is_modify'] == false) {
    ...
    function deleteservice() {
        ...
        if ($success) {
            ...
            $this->session->set_flashdata('success', 'This record has been deleted');
            redirect(base_url() . 'service/servicelist/'.$segment_id);
        } else {
            $this->session->set_flashdata('fail', 'This record has not been deleted');
            redirect(base_url() . 'service/servicelist/'.$segment_id);
        }
   ...
   //  die("ddddd");
    ...
    // For service Upgrade Form Data
    function serviceupgradeFormList() {
     $edit_id = $this->uri->segment(3);
      $skillId =  $this->servicemodel->serviceUpgradeFormlist($edit_id);
        ...
        $this->view_data['custumer_form_list'] = $this->servicemodel->serviceUpgradeFormlist($edit_id) ;
        
        $segment_id= $this->session->userdata('segment_id'); 
       
        //$this->view_data['service_list']=$this->servicemodel->getServiceList();
        $this->view_data['segment_id'] = $segment_id;
       $this->view_data['segment_list'] = $this->servicemodel->getServiceListExistFormServiceUpgrade();
        $this->view_data['skill_id'] = $edit_id;
        $this->view_data['edit_id'] = $this->uri->segment(3);
   
       // $this->view_data['serviceType'] = $serviceType;
        $this->load->view('header_category_list', $this->view_data);
        $this->load->view('serviceupgradeFormlist', $this->view_data);
        $this->load->view('footer');
    }
