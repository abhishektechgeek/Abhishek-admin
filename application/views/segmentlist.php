...
<p class="info" id="info"><span class="info_inner">Lorem ipsum dolor sit amet, consectetuer adipiscing elit</span></p>
    ...
    <div class="portlet">
        ...
        <?php if($is_modify){?> <a href="<?php echo base_url().'segment/edit_segment'?>" class="edit_inline">Add New</a> <?php }?>
        </div>
       
		<div class="portlet-content nopadding">
        <form action="<?php echo base_url().'segment/all_segment'?>" method="post">
          <table width="100%" cellpadding="0" cellspacing="0" id="box-table-a" summary="Employee Pay Sheet">
            <thead>
              <tr>
                <th width="34" scope="col"><input type="checkbox" name="allbox" id="allbox" onclick="checkAll()" /></th>
                <th width="206" scope="col">segment Name</th>
            ...
            <tbody>
             <?php $total=0;
                   $i=0;
					...
					$query2 = $this->db->query("SELECT * FROM `skills` WHERE id = '".$row->parent_id."'");
                    $row2 = $query2->row();
					?>
 					...
 					<tr>
                        ...
                        <td><?php echo $row2->name.'  > '.$row->name;?></td>
                          <td width="90">
                          <?php if($is_modify){
						  if($row->status == 1){?>
