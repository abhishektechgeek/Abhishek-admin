...
<!-- CONTENT START -->
    ...
    <!--  TITLE START  --> 
    <div class="grid_9">
      <h1 class="service_plans_icon">response</h1>
    </div>
...
<p class="info" id="info"><span class="info_inner">Lorem ipsum dolor sit amet, consectetuer adipiscing elit</span></p>
    ...
    <div class="portlet-header fixed error">
        ...
        <div class="portlet-header fixed"><img src="<?php echo base_url().'layout/';?>images/icons/user.gif" width="16" height="16" alt="Latest Registered Users" /> response Table Example</div>
        <div class="portlet-header fixed"  align="right"> 
        <?php if($is_modify){?> <!--<a href="<?php echo base_url().'response/edit_response'?>" class="edit_inline">Add New</a>--><?php }?>
        </div>
       
		<div class="portlet-content nopadding">
        <form action="<?php echo base_url().'response/all_response'?>" method="post">
            ...
            <thead>
              <tr>
                ...
                <th width="20%" scope="col">Service Provider Name</th>
                 <th width="10%" scope="col">Avilabilty</th>
                 <th width="10%" scope="col">Time</th>
                 <th width="10%" scope="col">Price</th>
                  <th width="20%" scope="col">Comment</th>
              </tr>
            </thead>
            <tbody>
             <?php $total=0;
                   $i=0;
				    foreach ($responselist->result() as $row) { 
  					/*$count = $this->responsemodel->getResponseCountBYBidId($row->id);
					$response = $this->responsemodel->getResponseDataBYBidId($row->id);*/
   				/*	echo "<pre>";print_r($response); */
  					...
  					<tr>
                        ...
                        <td width="10%"><?php echo $row->bid_id;?></td>
                        <td width="10%"><?php echo $row->service_provider_name;?></td>
                        <td width="20%"><?php echo $row->avilabilty;?></td>
                        <td width="20%"><?php echo $row->time;?></td>
                       <td width="20%"><?php echo $row->price;?></td>
                       <td width="20%"><?php echo $row->comment;?></td>
                           <td width="20%">
                          <?php 
						  $is_modify = true;
						if($is_modify){
						  if($row->approved == 1){?>
                           <!--<a href="<?php echo base_url().'response/inactiveresponse/'.$row->id;?>" title="Reject" class="reject_icon"></a>-->
                          <?php }else{?>
                          <!-- <a href="<?php echo base_url().'response/activeresponse/'.$row->id;?>" class="approve_icon" title="Approve"></a> -->
                          <?php }?> 
							 
                          <!-- <a href="<?php echo base_url().'response/deleteresponse/'.$row->id;?>" onclick="return confirm('Do you want to delete?');"" class="delete_icon" title="Delete"></a> -->
