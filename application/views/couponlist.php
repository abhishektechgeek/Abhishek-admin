...
<!-- CONTENT START -->
    ...
    <!--  TITLE START  --> 
    <div class="grid_9">
      <h1 class="service_plans_icon">Coupon</h1>
    </div>
...
<p class="info" id="info"><span class="info_inner">Lorem ipsum dolor sit amet, consectetuer adipiscing elit</span></p>
    ...
    <div class="portlet">
        ...
        <div class="portlet-header fixed"  align="right"> 
         <?php if($is_modify){?> <a href="<?php echo base_url().'coupon/edit_coupon'?>" class="edit_inline">Add New</a><?php }?>
        </div>
       
		<div class="portlet-content nopadding">
        <form action="<?php echo base_url().'coupon/all_coupon'?>" method="post">
          <table width="100%" cellpadding="0" cellspacing="0" id="box-table-a" summary="Employee Pay Sheet">
            <thead>
              <tr>
                <th width="10%" scope="col"><input type="checkbox" name="allbox" id="allbox" onclick="checkAll()" /></th>
                <th width="10%" scope="col">Coupon Name</th>
                <th width="10%" scope="col">Code</th>
                <th width="15%" scope="col">Discount</th>
                <th width="20%" scope="col">Date Start</th>
                <th width="25%" scope="col">Date End</th>
             ...
             <?php $total=0;
                   $i=0;
					...
					?>
 					...
 					<tr>
                        <td ><label>
                            <input type="checkbox" name="checkbox[]" id="checkbox[]" class="checkbox" value="<?php echo $row->coupon_id;?>"/>
                        </label></td>
						  ...
						  if($is_modify){
						   if($row->status == 1){?>
                           <a href="<?php echo base_url().'coupon/inactivecoupon/'.$row->coupon_id;?>" title="Reject" class="reject_icon"></a>
                          <?php }else{?>
                           <a href="<?php echo base_url().'coupon/activecoupon/'.$row->coupon_id;?>" class="approve_icon" title="Approve"></a> 
                          <?php }?> 
							 
                    
                         <a href="<?php echo base_url().'coupon/edit_coupon/'.$row->coupon_id;?>" class="edit_icon" title="Edit"></a> <a href="<?php echo base_url().'coupon/deletecoupon/'.$row->coupon_id;?>" onclick="return confirm('Do you want to delete?');"" class="delete_icon" title="Delete"></a></td>
              </tr>
					<?php }
					}
              ?>
