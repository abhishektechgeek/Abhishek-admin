<?php //echo "<pre>";print_r($bidslist);die;?>
...
<!-- CONTENT START -->
    ...
    <!--  TITLE START  --> 
    <div class="grid_9">
      <h1 class="service_plans_icon">notification</h1>
    </div>
...
<p class="info" id="info"><span class="info_inner">Lorem ipsum dolor sit amet, consectetuer adipiscing elit</span></p>
    ...
    <div class="portlet-header fixed error">
        ...
        <div class="portlet-header fixed"><img src="<?php echo base_url().'layout/';?>images/icons/user.gif" width="16" height="16" alt="Latest Registered Users" /> notification Table Example</div>
        <div class="portlet-header fixed"  align="right"> 
        <?php if($is_modify){?> <!--<a href="<?php echo base_url().'notification/edit_notification'?>" class="edit_inline">Add New</a>--><?php }?>
        </div>
       
		<div class="portlet-content nopadding">
        <form action="<?php echo base_url().'notification/all_notification'?>" method="post">
          <table width="100%" cellpadding="0" cellspacing="0" id="box-table-a" summary="Employee Pay Sheet">
            <thead>
              <tr>
                <th width="10%" scope="col"><input type="checkbox" name="allbox" id="allbox" onclick="checkAll()" /></th>
                <th width="10%" scope="col">Bid Number</th>
                <th width="10%" scope="col">Service Name</th>
                 <th width="20%" scope="col">Service Provider Name</th>
                 <th width="10%" scope="col">Has Response</th>
                 <th width="10%" scope="col">Approved</th>
            ...
            <tbody>
             <?php $total=0;
                   $i=0;
				    foreach ($notificationlist->result() as $row) { 
  					/*$count = $this->notificationmodel->getResponseCountBYBidId($row->id);
					$response = $this->notificationmodel->getResponseDataBYBidId($row->id);*/
   				/*	echo "<pre>";print_r($response); */
  					?>
  					<tr>
                        <td width="10%"><label>
                           <!-- <input type="checkbox" name="checkbox[]" id="checkbox[]" class="checkbox" value="<?php echo $row->menu_id;?>"/>-->
                           <input type="checkbox" name="checkbox[]" id="checkbox[]" class="checkbox" value="<?php echo $row->id;?>"/>
                        </label></td>
                        <td width="10%"><?php echo $row->bids_id;?></td>
                        <td width="10%"><?php echo $row->name;?></td>
                        <td width="20%"><?php echo $row->service_provider_name;?></td>
                        <td width="20%"><?php echo $row->is_response;?></td>
                       
                          <td width="20%"><?php echo $row->approved;?></td>
						...
						if($is_modify){
						  if($row->approved == 1){?>
                           <a href="<?php echo base_url().'notification/inactivenotification/'.$row->id;?>" title="Reject" class="reject_icon"></a>
                          <?php }else{?>
                           <a href="<?php echo base_url().'notification/activenotification/'.$row->id;?>" class="approve_icon" title="Approve"></a> 
                          <?php }?> 
							 
                           <a href="<?php echo base_url().'notification/deletenotification/'.$row->id;?>" onclick="return confirm('Do you want to delete?');"" class="delete_icon" title="Delete"></a> 
