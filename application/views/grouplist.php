...
<p class="info" id="info"><span class="info_inner">Lorem ipsum dolor sit amet, consectetuer adipiscing elit</span></p>
    ...
    <div class="portlet">
       ...
       <?php if($is_modify){?>  <a href="<?php echo base_url().'group/edit_group'?>" class="edit_inline">Add New</a><?php }?>
        </div>
       
		<div class="portlet-content nopadding">
        <form action="<?php echo base_url().'group/all_group'?>" method="post">
          <table width="100%" cellpadding="0" cellspacing="0" id="box-table-a" summary="Employee Pay Sheet">
          
            <thead>
              <tr>
                <th width="10%" scope="col"><input type="checkbox" name="allbox" id="allbox" onclick="checkAll()" /></th>
                <th width="80%" scope="col">group Name</th>
             ...
             <?php $total=0;
                   $i=0;
				    foreach ($skillslist_detail->result() as $row) { $i++;?>
 					...
 					<tr>
                        <td width="10%"><label>
                            <input type="checkbox" name="checkbox[]" id="checkbox[]" class="checkbox" value="<?php echo $row->group_id;?>"/>
                        </label></td>
                        <td width="80%"><?php echo $row->group_name;?></td>
						 ...
						 if($is_modify){
						  if($row->status == 1){?>
                           <a href="<?php echo base_url().'group/inactivegroup/'.$row->group_id;?>" title="Reject" class="reject_icon"></a>
                          <?php }else{?>
                           <a href="<?php echo base_url().'group/activegroup/'.$row->group_id;?>" class="approve_icon" title="Approve"></a> 
                          <?php }?> 
                      
                         <a href="<?php echo base_url().'group/edit_group/'.$row->group_id;?>" class="edit_icon" title="Edit"></a> <a href="<?php echo base_url().'group/deletegroup/'.$row->group_id;?>" onclick="return confirm('Do you want to delete?');"" class="delete_icon" title="Delete"></a></td>
              </tr>
					<?php }
					 }
              ?>
              ...
              <tr class="footer">
                <td colspan="4">
                <?php if($is_modify){?>
                <!--<a href="<?php echo base_url().'group/edit_group'?>" class="edit_inline">Add New</a>-->
