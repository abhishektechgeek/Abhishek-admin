...
<div class="grid_16" id="content">
    <!--  TITLE START  --> 
    <div class="grid_9">
        <h1 class="skills">Upgrade Service</h1>



        <?php if(!empty($edit_data[0]['service_id'])){
        ...
        <?php echo $row1[0]->category . ' > ' . $row1[0]->segment . ' > <b>' . $row1[0]->service; } else { }?>
    </div>
...
<?php if ($is_modify) { ?><a href="<?php echo base_url() . 'service/edit_subservices/' ?>" class="edit_inline">Add New</a><?php } ?>
            </div>
...
<?php } ?>
                ...
                <form action="<?php echo base_url() . 'service/all_service' ?>" method="post">
                    <table width="100%" cellpadding="0" cellspacing="0" id="box-table-a" summary="Employee Pay Sheet">
                        ...
                        <tbody>
                            ...
                            $total = 0;
                            $i = 0;
                            foreach ($skillslist_detail->result() as $row) {
                                $i++;
                                $query = $this->db->query("SELECT c.name AS service,u.attribute_name as subservice_name, p.name AS segment, (SELECT name FROM `skills` WHERE id = p.parent_id) AS category FROM `skills` AS c 
                                                                  LEFT JOIN `skills` p ON c.parent_id = p.id 
                                                                  LEFT JOIN `upgrade_option` u ON u.service_id = c.id
                                                                  WHERE c.id ='" . $row->id . "'");
    ...
    <?php if (!empty($s)) { ?>
                                    <tr>
                                        ...
                                        <td><?php foreach($row1 as $row2) { echo $row2->subservice_name.'<b>|</b>'; } ?></td>

                                        <td><?php echo $row1[0]->category . ' > ' . $row1[0]->segment . ' > ' . $row1[0]->service; ?></td>

                                        <td> <a href="<?php echo base_url() . 'service/view_subservices/' . $row->id; ?>"  title="View">View</a></td>
                                        <td width="90">
                                            <?php
                                            if ($is_modify) {
                                                if ($row->status == 1) {
            ...
            <?php } ?> 
                                                ...
                                                <a href="<?php echo base_url() . 'service/edit_subservices/' . $row->id; ?>" class="edit_icon" title="Edit"></a> <a href="<?php echo base_url() . 'service/delete_subservice/' . $row->id; ?>" onclick="return confirm('Do you want to delete?');" class="delete_icon" title="Delete"></a></td>
