...
class formLoader {
	...
	public function render_form()
	{
		...
		foreach ($this->form_data->fields as $field) {
			...
			// Label
			if($field->type == 'element-label') {
				$fields .= $this->element_label($field);
			}
                        // year
			if($field->type == 'element-year') {
				$fields .= $this->element_year($field);
			}
                        // year Only
			if($field->type == 'element-year-only') {
				$fields .= $this->element_year_only($field);
			}
	...
	private function element_checkboxes($field)
	{
		...
		for($i=0; $i <count($field->choices); $i++) {
			...
			$html .= '<div c><label>';
			$html .= sprintf('%s :%s', $i+1,$field->choices[$i]->value);
			$html .= '</label></div>';
		}
	...
	private function element_section_break($field)
	{
        ...
        private function element_label($field)
	{
               $html = sprintf('<label for="%s">%s </label>', $field->id, $field->title);
	 ...
	 * @access private
	 **/
        private function element_year($field){
		...
		//$html .= $this->make_label($id, $field->title, $required);
                ...
                $html = sprintf('<div><label for="%s">%s </label>:%s</div>', $field->id, $field->title ,$field->value);
		//$html .= '</div>';
	       return $html;
	}
        /**
	 ...
	 * @access private
	 **/
        private function element_year_only($field){
		...
		//$html .= $this->make_label($id, $field->title, $required);
                ...
                $html = sprintf('<div><label for="%s">%s </label>:%s</div>', $field->id, $field->title ,$field->value_year);
		//$html .= '</div>';
	       return $html;
	}
...
//var url ='http://gofavo.com/api/admin/sudf/exist.php?skill_id='+id;
   $("#target").attr("action", "javascript:var w=window.open('http://gofavo.com/api/admin/sudf/exist.php?skill_id="+id+"&edit_id="+<?php echo $edit_id ; ?>+"', '_blank', 'width=900,height=400, top=100px , left=100px , menubar=no ,titlebar=no,toolbar=no');");
   $( "#target" ).submit();
}
...
<!-- CONTENT START -->
    ...
    <!--  TITLE START  --> 
    <div class="grid_9">
    <h1 class="users">Service Upgrade Form</h1>
    </div>
    <!--RIGHT TEXT/CALENDAR-->
    ...
    <div class="clear">
     ...
     <div class="portlet-header fixed"  align="right"> 
      ...
      //  }
        ...
        if($is_modify){?><a href="javascript:var w=window.open('http://gofavo.com/api/admin/sudf?skill_id='+<?php echo $skill_id ; ?>+'&edit_id='+<?php echo $edit_id ; ?>, '_blank', 'width=900,height=400, top=100px , left=100px , menubar=no ,titlebar=no,toolbar=no');"   class="edit_inline">Edit Form</a>
            <a href="<?php echo base_url().'service/servicelistBackButton/'.$_SESSION['segment_id']?>" class="edit_inline">Back</a>
            <?php }?>
            
           
<section class="round-border">
			<div>
				Want to update with Existing Form<div><button href="#collapse1" class="nav-toggle">Click here</button></div>
			</div>
