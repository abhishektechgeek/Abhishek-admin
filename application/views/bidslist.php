...
<!-- CONTENT START -->
    ...
    <!--  TITLE START  --> 
    <div class="grid_9">
      <h1 class="service_plans_icon">Bids</h1>
    </div>
...
<p class="info" id="info"><span class="info_inner">Lorem ipsum dolor sit amet, consectetuer adipiscing elit</span></p>
    ...
    <div class="portlet-header fixed error">
        ...
        <div class="portlet-header fixed"><img src="<?php echo base_url().'layout/';?>images/icons/user.gif" width="16" height="16" alt="Latest Registered Users" /> bids Table Example</div>
        <div class="portlet-header fixed"  align="right"> 
        <?php if($is_modify){?> <a href="<?php echo base_url().'bids/edit_bids'?>" class="edit_inline">Add New</a><?php }?>
        </div>
       
		<div class="portlet-content nopadding">
        <form action="<?php echo base_url().'bids/all_bids'?>" method="post">
          <table width="100%" cellpadding="0" cellspacing="0" id="box-table-a" summary="Employee Pay Sheet">
            <thead>
              <tr>
                ...
                <th width="10%" scope="col">User Name</th>
                 <th width="20%" scope="col">Service Name</th>
                 <th width="10%" scope="col">Bid Response</th>
            ...
            <tbody>
             <?php $total=0;
                   $i=0;
				    foreach ($bidslist->result() as $row) { 
  					$count = $this->bidsmodel->getResponseCountBYBidId($row->id);
					$response = $this->bidsmodel->getResponseDataBYBidId($row->id);
   				/*	echo "<pre>";print_r($response); */
  					...
  					<tr>
                        ...
                        <td width="10%"><?php echo $row->first_name.'  '.$row->last_name;?></td>
                        <td width="20%"><?php echo $row->name;?></td>
                        <td width="20%"><?php 
 						    echo $count;
					    ?> </td>
                           <td width="10%"><?php if(isset($response['approved']) and $response['approved'] == 1) echo 'Approved';else echo 'Pending ';?></td>
                          <td width="10%"><?php if(isset($response['price'])) echo $response['price'];else echo '0';?></td>
						...
						if($is_modify){
						  if($row->status == 1){?>
                           <a href="<?php echo base_url().'bids/inactivebids/'.$row->id;?>" title="Reject" class="reject_icon"></a>
                          <?php }else{?>
                           <a href="<?php echo base_url().'bids/activebids/'.$row->id;?>" class="approve_icon" title="Approve"></a> 
                          <?php }?> 
							 
                          <a href="<?php echo base_url().'bids/edit_bids/'.$row->id;?>" class="edit_icon" title="Edit"></a> <a href="<?php echo base_url().'bids/deletebids/'.$row->id;?>" onclick="return confirm('Do you want to delete?');"" class="delete_icon" title="Delete"></a> 
              <a href="<?php echo base_url().'bids/view_bids/'.$row->id;?>">View</a>
					<?php 	 }
					$i++;
					}
              ?>
