...
<!-- CONTENT START -->
    ...
    <!--  TITLE START  --> 
    <div class="grid_9">
      <h1 class="service_plans_icon">Admins</h1>
    </div>
...
<p class="info" id="info"><span class="info_inner">Lorem ipsum dolor sit amet, consectetuer adipiscing elit</span></p>
    ...
    <div class="portlet-header fixed error">
        ...
        <div class="portlet-header fixed"><img src="<?php echo base_url().'layout/';?>images/icons/user.gif" width="16" height="16" alt="Latest Registered Users" /> admins Table Example</div>
        <div class="portlet-header fixed"  align="right"> 
        <?php if($is_modify){?> <a href="<?php echo base_url().'admins/edit_admins'?>" class="edit_inline">Add New</a><?php }?>
        </div>
       
		<div class="portlet-content nopadding">
        <form action="<?php echo base_url().'admins/all_admins'?>" method="post">
          <table width="100%" cellpadding="0" cellspacing="0" id="box-table-a" summary="Employee Pay Sheet">
            ...
            <tbody>
             <?php $total=0;
                   $i=0;
				    foreach ($adminslist->result() as $row) { $i++;
 					?>
 					...
 					<tr>
                        ...
                        <td width="70%"><?php echo $row->first_name.'  '.$row->last_name;?></td>
                        <td><?php 
						...
						if($is_modify){
						  if($row->status == 1){?>
                           <a href="<?php echo base_url().'admins/inactiveadmins/'.$row->id;?>" title="Reject" class="reject_icon"></a>
                          <?php }else{?>
                           <a href="<?php echo base_url().'admins/activeadmins/'.$row->id;?>" class="approve_icon" title="Approve"></a> 
                          <?php }?> 
							 
                          <a href="<?php echo base_url().'admins/edit_admins/'.$row->id;?>" class="edit_icon" title="Edit"></a> <a href="<?php echo base_url().'admins/deleteadmins/'.$row->id;?>" onclick="return confirm('Do you want to delete?');"" class="delete_icon" title="Delete"></a> 
