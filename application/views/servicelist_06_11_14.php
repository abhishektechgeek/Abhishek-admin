...
{ 
alert('shdjgf');
$("#imageform").attr("action", "<?php echo base_url().'service/serviceUpdateImage'?>");
$( "#imageform" ).submit();
});
...
<p class="info" id="info"><span class="info_inner">Lorem ipsum dolor sit amet, consectetuer adipiscing elit</span></p>
    ...
    <div class="portlet">
        ...
        <form action="<?php echo base_url().'service/all_service'?>" method="post">
          <table width="100%" cellpadding="0" cellspacing="0" id="box-table-a" summary="Skills List">
            <thead>
              <tr>
                ...
                <th width="206" scope="col">service Name</th>
                <th width="206" scope="col">service Type</th>
                <th width="206" scope="col">service Image</th>
                <th width="200" scope="col">Time Slot</th>
                <th width="200" scope="col">Time Interval</th>
                <!--th width="206" scope="col">Sub service Name</th-->
                <th width="282" scope="col">Path</th>
                <th width="282" scope="col">Add Sub Services</th>
                <th width="282" scope="col">View</th>
                   ...
                   $i=0;
				    foreach ($skillslist_detail->result() as $row) { $i++;
 					...
 					<tr>
                        ...
                        <td style="width:200px;" class="wrapword">
                            <form id='imageform' method='post' enctype='multipart/form-data' action='' >
                                <table >
                            <tr>
                            <td style="width:200px;" class="wrapword"><input type='file' name='photoimg' id='photoimg' onchange="showimagepreview(this)" /><input type="hidden" value=<?php echo $row->id ?> name="service_id" /></td>
                            <td style="width:80px;" class="wrapword"><img style="height: 80px;width: 80px;" id="preview" alt="uploaded image preview" src ="<?php echo $row->image==""?"http://gofavo.com/api/admin/application/uploads/default.jpg":"http://gofavo.com/api/admin/application/uploads/".$row->image;?>"/></td>
        ...
        ?></td>
                        ...
                        <td> <a href="<?php echo base_url().'serviceupgrade/index/'.$row->id;?>"  title="Add Sub Service">View Sub Service</a></td>
                       <td> <a href="<?php echo base_url().'service/view_service/'.$row->id;?>"  title="View">View</a></td>
                          <td width="90">
                          <?php 
						  if($is_modify){
						  if($row->status == 1){?>
                           <a href="<?php echo base_url().'service/inactiveservice/'.$row->id;?>" title="Reject" class="reject_icon"></a>
                          <?php }else{?>
                           <a href="<?php echo base_url().'service/activeservice/'.$row->id;?>" class="approve_icon" title="Approve"></a> 
                          <?php }?> 
							 
                    
                         <a href="<?php echo base_url().'service/edit_service/'.$row->id;?>" class="edit_icon" title="Edit"></a> <a href="<?php echo base_url().'service/deleteservice/'.$row->id;?>" onclick="return confirm('Do you want to delete?');" class="delete_icon" title="Delete"></a></td>
