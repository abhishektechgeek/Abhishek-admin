<?php
class Serviceupgrademodel extends CI_Model {
	...
	function servicelist($action='',$start_row='',$per_page='',$order_field='',$order_seq=''){
		...
		$segment_id = $this->input->post('segment_id'); 
 		...
 		if($segment_id > 0){
		   ...
		   $query_str = "SELECT * FROM `skills` WHERE type = 4 ";
		}
...
//		if($res){  return true; }else{ return false; }
	...
	if($i == 1){
 	 $insert .= "  VALUES (NULL, '".$this->input->post('attribute_name_'.$i)."', '".mysql_escape_string($attr['description'])."','".$image."','','".$id."')";
 	  }
	else{ 
	...
	function getSegmentList(){
 		 $query = $this->db->query("SELECT c.id, c.name, p.name as parentName FROM `skills` AS c INNER JOIN `skills` AS p ON c.parent_id = p.id WHERE c.type = '3'");
 ...
 function getServiceList2(){
       echo $id=$this->input->post('segment_id');
     if(isset($id)){
      $query = $this->db->query("SELECT c.id , c.name AS serviceupgrade , p.name AS service, p1.name AS segment,p2.name AS category
						FROM `skills` AS c
	...
	function getSegmentById($id){
		...
		$query_str=$this->db->query("SELECT c.id, c.name, p.name as parentName FROM `skills` AS c WHERE c.type = '2'
     	...
     	return $row;
        	...
        	function save_serviceupgrade($data, $service_id){
	...
	if(!$id>0){   
			...
			//'specification'=>$this->input->post('specification'),
		      ...
		      $res= $this->db->update('skills', $data);	
                ...
                function deleteserviceupgrade($id){
	  	$where['id']=$id;	
  		$this->db->where($where);
		$res= $this->db->delete('skills'); 
  		if($res){return true; }else{ return false; }
