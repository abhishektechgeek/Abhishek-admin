...
function userlist($page,$offset,$type='0',$like = false){
	...
	function userCount($type='0'){
  	    $result = array();
		if($type=='0')
 		   $where = "WHERE 1";
		else 
		   $where = "WHERE u2.type != '".$type."'";
   ...
   function getAccessMenuList(){
        $level= $this->session->userdata('level');
		 if($level == 0){
		 $query3 = $this->db->query("SELECT menu_id FROM `menu` ");
		 $row3 = $query3->result_array();
		
		 $access_permission = array();
		 foreach($row3 as $key=>$value){
			 $access_permission[] = $value['menu_id'];
		 }
...
function getUserBidList($id){
 $query = $this->db->query("SELECT u.first_name ,u.last_name ,s.name ,b.job_description ,b.my_availability ,b.my_availability , b.time_slot1,ul.type,bn.approved FROM skills s INNER JOIN bids b ON s.id = b.service_id INNER JOIN users u ON u.id = b.user_id INNER JOIN users_level ul ON u.id = ul.user_id  INNER JOIN bids_notification bn ON b.id = bn.bids_id WHERE u.id = '".$id."' ");
	   $row  = $query->result_array();
 	   $bidsList = array();
...
function getUserSkillsList($id){
 $query = $this->db->query("SELECT s.name FROM skills s INNER JOIN service_provider_skills_map sp ON s.id = sp.skills_id  WHERE sp.service_provider_id = '".$id."' ");
	   $row  = $query->result_array();
 	   $skillsList = array();
		 foreach($row as $key=>$value){
			 $skillsList[] = $value;
 ...
 function fatch_top_bids(){
$query = $this->db->query("SELECT b.id,u.first_name ,u.last_name ,s.name ,b.job_description ,b.my_availability ,b.my_availability , b.time_slot1,ul.type,bn.approved FROM skills s INNER JOIN bids b ON s.id = b.service_id INNER JOIN users u ON u.id = b.user_id INNER JOIN users_level ul ON u.id = ul.user_id  INNER JOIN bids_notification bn ON b.id = bn.bids_id GROUP BY b.id DESC LIMIT 0,10 ");
 ...
 function fatch_top_users(){
$query = $this->db->query("SELECT u.id ,u.first_name ,u.last_name ,ul.type FROM users u  LEFT JOIN users_level ul ON u.id = ul.user_id  ORDER BY u.id DESC LIMIT 0,5 ");
	   $row  = $query->result_array();
 	   $usersList = array();
		 foreach($row as $key=>$value){
			 $usersList[] = $value;
...
function fetch_total_bids(){
$query = mysql_query(" SELECT count( id ) FROM bids ");
$result = mysql_fetch_array($query);
$ids = $result[0];
           return $ids;
 }
 ...
 function fetch_total_sales(){
$query = mysql_query(" SELECT SUM(price) FROM service_plan ");
$result = mysql_fetch_array($query);
$ids = $result[0];
           return $ids;
 }
