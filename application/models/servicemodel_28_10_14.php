	...
	function getSegmentById($id){
		...
		$query_str=$this->db->query("SELECT c.id, c.name, p.name as parentName FROM `skills` AS c WHERE c.type = '2'
     	...
     	return $row;
        	...
        	function save_service_upgrade($data, $serviceId){
   		//print_r($data);
                //echo  $serviceId;
                $this->db->where(array('parent_id' => $serviceId));
                $count=$this->db->count_all_results('skills');
               
  		if($count==0){
                      foreach ($data as $value) {
                        
                        $result= $this->db->insert('skills', $value);
                        if($result){
                            return true;
                        }
                        else{ 
                    ...
                    //    $result= $this->db->update('skills', $value);
                        if($result){
                        return true;
                        }
                        else{ return false; }
        ...
        function updatetimeslot($id,$time,$timeInterval){
          echo $query_str="UPDATE  skills	SET timeslot = '$time' ,time_interval = '$timeInterval' WHERE id = '".$id."' ";
