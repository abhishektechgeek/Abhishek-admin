...
function isUserExistsInContactList($id){
              $query2 = "SELECT `id` FROM `serviceupgrade_form_data` WHERE s_id = '".$id."' "; 
...
//echo $form_data ;
     ...
     //isUserExistsInContactList($s_id)
      if($s_id!=0){
          $id=$s_id;
        $query2 = "update serviceupgrade_form_data set form_data= '$form_data' where s_id = $e_id "; 
        $result = mysql_query($query2);
        }else{
            $id= $e_id;
       $query2 = "insert into serviceupgrade_form_data (s_id ,form_data) values($e_id,'$form_data')"; 
       $result = mysql_query($query2);
        }

//echo "<script>window.close();</script>";
        echo '<script type="text/javascript">self.close(); opener.location.href = "http://gofavo.com/api/admin/service/serviceupgradeFormList/'.$id.'";</script>';
