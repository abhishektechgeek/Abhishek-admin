	...
	$.fn.formBuilder = function(options) {
		...
		var fieldAdd = function() {
			...
			$('.new-element').click(function(){
				...
				dust.render(tpl, data, function(err, out) {
                 ...
                 $('.new-element-slider').click(function(){
				...
				dust.render(tpl, data, function(err, out) {
                 ...
                 $('.new-element-label').click(function(){
			...
			$('.form-element').click(function(){
				...
				if( $(this).data('type') == 'form-settings' ) {
					...
					currentlySelected = $(this);
                                       ...
                                       if($(this).data('type')=="element-year" || $(this).data('type')=="element-year-only"){
                                            bindSettings2();
                                        }else{
                                              bindSettings();
                                        }
					...
					repositionToolbox();
                                        if($(this).data('type') == 'element-label'){
                                            isFieldOptionsLabel();
                                        }else{
					isFieldOptions();
                                        }
		...
		*/
		var bindSettings = function() {	
			...
			// Field Label
                        $('#field-label').prop('disabled',false);
		...
		var bindSettings2 = function() {	
			// Field Label
			$('#field-label').val(currentlySelected.data('label'));
                        $('#field-label').prop('disabled',true);
			$('#field-label').on("keyup", function() { 
		...
		*/
		...
		var isFieldOptionsLabel = function() {
                    
			if( currentlySelected.data('type') == 'element-label' ) {
				$('#field-options').hide();
				$('#field-description').hide();
			} else {
				$('#field-options').hide();
		...
		var serialize = function() {
			...
			$('#sortable-elements li').each(function(i){
				...
				var element = {
					...
					'position': i+1,
					'description': $(this).data('type')=="element-year"?"":$(this).data('description'),
                                        'value':$(this).data('type')=="element-year"?$(this).find('#year').val():"",
                                        'value_year':$(this).data('type')=="element-year-only"?$(this).find('#year-only').val():""
				}
